...
/**
 * Save audio buffer to temporary storage as MP3 file
 * Returns the file path
 */
async function saveAudioToTemp(callId: string, mp3Buffer: Buffer): Promise<string> {
  try {
    // Ensure temp directory exists
    const tempDir = '/tmp/outbound-audio';
    await mkdir(tempDir, { recursive: true });
    
    // Save MP3 directly - no transcoding or headers needed
